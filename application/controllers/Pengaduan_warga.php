...
class Pengaduan_warga extends MY_Controller {
    ...
    public function ajax_list()
    {
        ...
        set_time_limit(3600);
        $list = $this->Mod_pengaduan_warga->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Mod_pengaduan_warga->count_all(),
                        "recordsFiltered" => $this->Mod_pengaduan_warga->count_filtered(),
                        "data" => $data,
                );
    ...
    public function konfirmasi_pengaduan()
    {
        ...
        $this->Mod_pengaduan_warga->konfirmasi_pengaduan($id, $save);
    ...
    function view_pengaduan(){
        // get personil
        $id = $this->input->post('id');
        $data_pengaduan = $this->Mod_pengaduan_warga->get_pengaduan($id);
    ...
    public function delete_laporan(){
        $id = $this->input->post('id');
        $g = $this->Mod_pengaduan_warga->get_image($id)->row_array();
        if ($g != null) {
            //hapus gambar yg ada diserver
            unlink('assets/foto/laporan/'.$g['image']);
        }
        $this->Mod_pengaduan_warga->delete_laporan($id, 'tbl_laporan');
        $data['status'] = TRUE;
